            ...
            <AnimatePresence>
              {items.map((item, i) => (
                <motion.div key={item.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.03 }} className={`bg-surface p-4 flex gap-3 rounded-2xl shadow-sm border border-surface-200 transition-colors duration-300 ${!item.isAvailable ? 'opacity-50 grayscale' : ''}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-foreground/40 text-xs font-medium">{item.categoryName}</p>
                      </div>
                      <span className="font-black text-brand-500 text-sm flex-shrink-0">ETB {Number(item.price).toFixed(0)}</span>
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      {item.isFasting && <span className="badge-fasting text-[10px]">{t('fasting')}</span>}
                      <div className="flex items-center gap-1 ml-auto">
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleToggle(item)}
                          className="p-1" id={`toggle-${item.id}`}>
                          {item.isAvailable
                            ? <ToggleRight size={24} className="text-emerald-500" />
                            : <ToggleLeft size={24} className="text-foreground/20" />}
                        </motion.button>
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => { setEditItem(item); setFormOpen(true); }}
                          className="w-8 h-8 rounded-lg bg-surface-100 flex items-center justify-center border border-surface-200" id={`edit-${item.id}`}>
                          <Pencil size={12} className="text-foreground/60" />
                        </motion.button>
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleDelete(item.id)}
                          className="w-8 h-8 rounded-lg bg-rose-500/10 flex items-center justify-center border border-rose-500/20" id={`delete-${item.id}`}>
